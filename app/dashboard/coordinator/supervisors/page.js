...
const Supervisors = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            show("error", "Fetch Failed", "Could not load supervisors.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const deleteApi = async (ids) => {
        ...
        const res = await fetch(`/api/coordinator/supervisors?${query}`, { method: "DELETE" });
        if (!res.ok) {
                ...
                accept: async () => {
                    ...
                    try {
                        ...
                        show(
                            "success",
                            "Deleted",
                            ids.length === 1 ? "Supervisor deleted." : `${ids.length} supervisors deleted.`
                        );
                        setSupervisors((prev) => prev.filter((u) => !ids.includes(u.id)));
    ...
    const handleUpdate = useCallback(
        (updated) => {
            ...
            setSupervisors((prev) => prev.map((u) => (u.id === transformed.id ? transformed : u)));
            setEdit(false);
            show("success", "Updated", "Supervisor details have been updated successfully.");
        },
        [show]
    );
    ...
    return (
        <section className="flex flex-col w-full py-3 px-4">
            ...
            {create && <NewSupervisor close={() => setCreate(false)} onCreated={fetchData} />}
            {edit && (
                <EditSupervisor
                    supervisor={current}
                    close={() => setEdit(false)}
            ...
            {view && <Supervisor supervisor={current} visible={view} onClose={() => setView(false)} />}

            <div className="bg-white rounded-md shadow-md space-y-4">
                <div className="flex justify-between items-center border-b px-3 py-2">
                    <h1 className="text-2xl font-bold text-gray-700">All Supervisors</h1>
