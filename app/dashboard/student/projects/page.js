...
const Projects = () => {
    ...
    const deleteApi = async (ids) => {
        ...
        const res = await fetch(`/api/student/projects?${query}`, { method: "DELETE" });
        if (!res.ok) {
            ...
            confirmDialog({
                ...
                accept: async () => {
                    ...
                    try {
                        ...
                        setProjects((prev) => prev.filter((u) => !ids.includes(u.id)));
    ...
    const handleUpdate = useCallback(
        (updated) => {
            ...
            setProjects((prev) => prev.map((u) => (u.id === transformed.id ? transformed : u)));
            setEdit(false);
            show("success", "Updated", "Project details have been updated successfully.");
        },
        [show]
    );
        ...
        <section className="flex flex-col w-full py-3 px-4">
            ...
            <div className="bg-white rounded-md shadow-md space-y-4">
                <div className="flex justify-between items-center border-b px-3 py-2">
                    <h1 className="text-2xl font-bold text-gray-700">All Projects</h1>
