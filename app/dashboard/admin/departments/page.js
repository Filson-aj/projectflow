...
const Departments = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            show("error", "Fetch Failed", "Could not load departments.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const deleteApi = async (ids) => {
        ...
        const res = await fetch(`/api/admin/departments?${query}`, { method: "DELETE" });
        if (!res.ok) {
                ...
                accept: async () => {
                    ...
                    try {
                        ...
                        show(
                            "success",
                            "Deleted",
                            ids.length === 1 ? "Department deleted." : `${ids.length} departments deleted.`
                        );
                        setDepartments((prev) => prev.filter((u) => !ids.includes(u.id)));
    ...
    // Merge-back handler for edits
    const handleUpdate = useCallback(
        (updated) => {
            setDepartments((prev) => prev.map((u) => (u.id === updated.id ? updated : u)));
            setEdit(false);
            show("success", "Updated", "Department details have been updated successfully.");
        },
        [show]
    );
    ...
    return (
        <section className="flex flex-col w-full py-3 px-4">
            ...
            {create && <NewDepartment close={() => setCreate(false)} onCreated={fetchData} />}
            {edit && (
                <EditDepartment
                    department={current}
                    close={() => setEdit(false)}
            ...
            <div className="bg-white rounded-md shadow-md space-y-4">
                <div className="flex justify-between items-center border-b px-3 py-2">
                    <h1 className="text-2xl font-bold text-gray-700">All Departments</h1>
